...
const views: Array<{
  ...
  { id: "dashboard", label: "首页", icon: BarChart3, group: "常用", roles: ["student", "teacher", "parent"] },
  { id: "tutor", label: "AI辅导", icon: MessageCircle, group: "常用", roles: ["student", "teacher", "parent"] },
  ...
  { id: "parent", label: "家长端", icon: Users, group: "管理", roles: ["parent"] },
  { id: "teacher", label: "老师端", icon: School, group: "管理", roles: ["teacher"] },
  { id: "about", label: "关于", icon: Sparkles, group: "系统", roles: ["student", "teacher", "parent"] },
  { id: "settings", label: "设置", icon: Settings, group: "系统", roles: ["student", "teacher", "parent"] }
] as const;
    ...
    <Box component="main" className={mobileRailOpen ? "chat-app rail-open" : "chat-app"}>
      ...
      <Paper component="section" className="chat-main" elevation={0}>
        ...
        {view === "tutor" ? (
          ...
          <LearningWorkspace
            view={view === "admin" ? "dashboard" : view}
            stats={stats}
...
function GradingReportView({ report }: { report: { score: number; summary?: string; correct_points?: string[]; suggestions?: string[]; wrong_points?: string[] } }) {
  return (
    <div className="grading-report">
      <strong>批改结果：{report.score}分</strong>
      {report.summary ? <MarkdownView>{`**正确答案 / 参考结论：**\n\n${report.summary}`}</MarkdownView> : null}
      {report.correct_points?.length ? <MarkdownView>{`**做对的地方：**\n${markdownBulletList(report.correct_points)}`}</MarkdownView> : null}
      {report.wrong_points?.length ? <MarkdownView>{`**错误原因：**\n${markdownBulletList(report.wrong_points)}`}</MarkdownView> : null}
      {report.suggestions?.length ? <MarkdownView>{`**如何避免：**\n${markdownBulletList(report.suggestions)}`}</MarkdownView> : null}
    </div>
  );
}
